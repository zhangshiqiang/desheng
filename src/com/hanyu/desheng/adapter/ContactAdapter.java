...
//        User user = getItem(position);
        User user = copyUserList.get(position);
        if (user == null)
        ...
        if (position == 0 || header != null
                && !header.equals(copyUserList.get(position - 1).getHeader())) {
            if ("".equals(header)) {
...
//        return super.getItem(position);
    	return copyUserList.get(position);
    }
	...
	public void onClick(View v) {
		...
		case R.id.avatar:
			...
			User user = getItem(position);
			Intent preIntent = new Intent(context, PerInfoAct.class);
//			preIntent.putExtra("phone", user.getMobile());
			preIntent.putExtra("hxuser", user.getUsername());
			context.startActivity(preIntent);
			break;
