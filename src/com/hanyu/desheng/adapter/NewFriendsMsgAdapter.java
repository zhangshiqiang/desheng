...
public class NewFriendsMsgAdapter extends ArrayAdapter<InviteMessage> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.user_reject = (Button) convertView.findViewById(R.id.user_reject);
		...
		if (msg != null) {
			...
			if (HxUserDao.findByHx(msg.getFrom()) != null) {
				holder.name.setText(HxUserDao.findByHx(msg.getFrom()).username);
				ImageLoader.getInstance().displayImage(HxUserDao.findByHx(msg.getFrom()).headpic, holder.avator,
						options);
			} else {
				List<String> list = new ArrayList<String>();
				list.add(msg.getFrom());
				getHxUserName(gson.toJson(list), holder.name, holder.avator);
			}
			...
			if (msg.getStatus() == InviteMesageStatus.BEAGREED) {
				...
				holder.user_reject.setVisibility(View.INVISIBLE);
				...
				holder.user_reject.setOnClickListener(new OnClickListener() {
					
					@Override
					public void onClick(View v) {
						rejectInvitation(holder.user_reject, msg);
					}
				});
	 ...
	 * 拒绝好友请求或者群申请
	 */
	private void rejectInvitation(final Button button, final InviteMessage msg) {
		final ProgressDialog pd = new ProgressDialog(context);
		String str1 = context.getResources().getString(R.string.Are_reject_with);
		final String str2 = context.getResources().getString(R.string.Has_refused_to);
		final String str3 = context.getResources().getString(R.string.reject_with_failure);
		pd.setMessage(str1);
...
//						EMGroupManager.getInstance().acceptApplication(msg.getFrom(), msg.getGroupId());
						EMGroupManager.getInstance().declineApplication(msg.getFrom(), msg.getGroupId(), "拒绝了您的加群申请");
					}
...
//							messgeDao.deleteMessage(msg.getFrom());
							objects = messgeDao.getMessagesList();
